...
class DiskBridgeDataset(Dataset):
    def __init__(
        self,
        datasets_dir,
        image_fn: Callable,
    ...
    def _load_episode(self, idx: int, window_size: int) -> Dict[str, np.ndarray]:
        start_idx = self.episode_lookup[idx]
        end_idx = start_idx + window_size
        keys = ['observation.images.image_0', 'observation.state', 'action']
    ...
    def _process_rgb(self, episode: Dict[str, np.ndarray]) -> Dict[str, Dict[str, torch.Tensor]]:
        rgb_obs = torch.from_numpy(episode['observation.images.image_0']) * 255
...
def get_bridge_dataset(args, image_processor, tokenizer, epoch=0, floor=False):
    # ann is dict including language and info
    shared_epoch = SharedEpoch(epoch=epoch)
    preprocess_image_fn = functools.partial(
        preprocess_image, image_processor=image_processor
    ...
    calvin_dataset = DiskBridgeDataset(
