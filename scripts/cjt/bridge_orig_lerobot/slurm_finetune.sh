...
### NEED TO CHANGE ###
calvin_dataset_path="/ailab/group/pjlab-smartbot/share/Official_Manipulation_Data/simpler_env_lerobot/bridge_orig_lerobot"
save_checkpoint_path="checkpoints/seer_bridge"
...
torchrun --nnodes="${NODES}" --node_rank="${NODE_RANK}" --nproc_per_node="${NPROC_PER_NODE}" --master_addr="${MASTER_ADDR}" --master_port="${MASTER_PORT}" train.py \
    ...
    --learning_rate 1e-3 \
    --save_checkpoint \
    --finetune_type "bridge" \
    --wandb_project seer \
    --weight_decay 1e-4 \
    --num_resampler_query 6 \
    --run_name finetune_bridge_ep5 \
